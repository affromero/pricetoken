      ...
      if (config.fallbackUrls?.length) {
        ...
        for (const fallbackUrl of config.fallbackUrls) {
          try {
            ...
            const fallbackExtraction = await extractSttPricing(providerId, fallbackText);
            let added = 0;
            ...
            if (added > 0) {
              console.log(`${config.displayName}: recovered ${added} new STT model(s) from ${fallbackUrl}`);
            }
            pageText += '\n\n---\n\n' + fallbackText;
          } catch {
