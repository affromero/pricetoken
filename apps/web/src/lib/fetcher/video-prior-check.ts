...
export async function checkVideoPriorConsistency(
  provider: string,
  models: ExtractedVideoModel[]
): Promise<PriorConsistencyFlag[]> {
  const lastSnapshots = await getLastVideoSnapshots(provider);
  ...
  for (const model of models) {
    ...
    if (!prior) {
      flags.push({
        ...
        type: 'new_model',
        detail: `New video model not seen before: ${model.displayName}`,
      });
      continue;
    }
  ...
  for (const prior of lastSnapshots) {
    if (!newModelIds.has(prior.modelId)) {
      flags.push({
        ...
        detail: `Video model ${prior.displayName} was in last snapshot but not in new extraction`,
      });
    }
  }
...
async function getLastVideoSnapshots(provider: string) {
