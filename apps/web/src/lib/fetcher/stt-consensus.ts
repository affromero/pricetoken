...
import type { VerifiedSttModel, SttVerificationResult } from './stt-verification-types';
import { shouldApprove } from './consensus';

export function buildSttConsensus(
  models: ExtractedSttModel[],
  agentResults: AgentVerification[],
  priorFlags: PriorConsistencyFlag[]
): SttVerificationResult {
  const approved: VerifiedSttModel[] = [];
  const flagged: VerifiedSttModel[] = [];
  ...
  for (const model of models) {
    ...
    const verified: VerifiedSttModel = {
      ...model,
