...
import type { VerifiedTtsModel, TtsVerificationResult } from './tts-verification-types';
import { shouldApprove } from './consensus';

export function buildTtsConsensus(
  models: ExtractedTtsModel[],
  agentResults: AgentVerification[],
  priorFlags: PriorConsistencyFlag[]
): TtsVerificationResult {
  const approved: VerifiedTtsModel[] = [];
  const flagged: VerifiedTtsModel[] = [];
  ...
  for (const model of models) {
    ...
    const verified: VerifiedTtsModel = {
      ...model,
