...
import type { VerifiedMusicModel, MusicVerificationResult } from './music-verification-types';
import { shouldApprove } from './consensus';

export function buildMusicConsensus(
  models: ExtractedMusicModel[],
  agentResults: AgentVerification[],
  priorFlags: PriorConsistencyFlag[]
): MusicVerificationResult {
  const approved: VerifiedMusicModel[] = [];
  const flagged: VerifiedMusicModel[] = [];
  ...
  for (const model of models) {
    ...
    const verified: VerifiedMusicModel = {
      ...model,
