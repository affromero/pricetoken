...
import type { VerifiedAvatarModel, AvatarVerificationResult } from './avatar-verification-types';
import { shouldApprove } from './consensus';

export function buildAvatarConsensus(
  models: ExtractedAvatarModel[],
  agentResults: AgentVerification[],
  priorFlags: PriorConsistencyFlag[]
): AvatarVerificationResult {
  const approved: VerifiedAvatarModel[] = [];
  const flagged: VerifiedAvatarModel[] = [];
  ...
  for (const model of models) {
    ...
    const verified: VerifiedAvatarModel = {
      ...model,
