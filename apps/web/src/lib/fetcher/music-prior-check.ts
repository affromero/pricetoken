...
export async function checkMusicPriorConsistency(
  provider: string,
  models: ExtractedMusicModel[]
): Promise<PriorConsistencyFlag[]> {
  const lastSnapshots = await getLastMusicSnapshots(provider);
  ...
  for (const model of models) {
    ...
    if (!prior) {
      flags.push({
        ...
        type: 'new_model',
        detail: `New music model not seen before: ${model.displayName}`,
      });
      continue;
    }
  ...
  for (const prior of lastSnapshots) {
    if (!newModelIds.has(prior.modelId)) {
      flags.push({
        ...
        detail: `Music model ${prior.displayName} was in last snapshot but not in new extraction`,
      });
    }
  }
...
async function getLastMusicSnapshots(provider: string) {
