...
export async function checkAvatarPriorConsistency(
  provider: string,
  models: ExtractedAvatarModel[]
): Promise<PriorConsistencyFlag[]> {
  const lastSnapshots = await getLastAvatarSnapshots(provider);
  ...
  for (const model of models) {
    ...
    if (!prior) {
      flags.push({
        ...
        type: 'new_model',
        detail: `New avatar model not seen before: ${model.displayName}`,
      });
      continue;
    }
  ...
  for (const prior of lastSnapshots) {
    if (!newModelIds.has(prior.modelId)) {
      flags.push({
        ...
        detail: `Avatar model ${prior.displayName} was in last snapshot but not in new extraction`,
      });
    }
  }
...
async function getLastAvatarSnapshots(provider: string) {
