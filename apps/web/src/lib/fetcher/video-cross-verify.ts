...
export async function videoCrossVerify(
  ...
  const userPrompt = `Verify these extracted video prices against the raw page text.\n\nRaw text:\n${pageText.slice(0, 6000)}\n\nExtracted data:\n${modelsJson}`;
  ...
  if (availableAgents.length === 0) {
    console.warn('No verification agents available — skipping video cross-verification');
    return [];
  }

  const results = await Promise.allSettled(
    availableAgents.map((agent) => runVideoVerificationAgent(agent, userPrompt))
  );
  ...
  for (const [i, result] of results.entries()) {
    if (result.status === 'fulfilled') {
      ...
      console.error(
        `Video verification agent ${agent.provider}/${agent.model} failed:`,
        result.reason
      );
    }
  }
...
async function runVideoVerificationAgent(
  ...
  logUsage({
    ...
    metadata: { operation: 'video_verification' },
  });
...
function parseVerdicts(text: string): ModelVerdict[] {
  try {
    ...
    console.warn('Failed to parse video verification verdicts:', text.slice(0, 200));
    return [];
  }
}
