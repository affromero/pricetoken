...
import { STT_SYSTEM_PROMPT } from './stt-system-prompt';

export interface ExtractedSttModel {
  modelId: string;
  displayName: string;
...
export interface SttExtractionOutput {
...
export async function extractSttPricing(
  pricingProvider: string,
  pageText: string
): Promise<SttExtractionOutput> {
  ...
  const userPrompt = `Extract ${pricingProvider} speech-to-text model pricing from this page:\n\n${truncated}`;

  const startTime = Date.now();
  let result: ExtractionResult;
  try {
    result = await extractionProvider.extract(apiKey, config.extractionModel, STT_SYSTEM_PROMPT, userPrompt);
  } catch (err) {
    logUsage({
      ...
      metadata: { pricingProvider, operation: 'stt_extraction' },
    });
    throw new Error(
      `STT extraction failed with ${config.extractionProvider}/${config.extractionModel}: ${err instanceof Error ? err.message : String(err)}`
    );
  }

  const models = parseSttModels(result.content, pricingProvider);

  logUsage({
    provider: config.extractionProvider,
    model: config.extractionModel,
    operation: 'stt_extraction',
...
function parseSttModels(text: string, pricingProvider: string): ExtractedSttModel[] {
  try {
    ...
    console.warn(`Failed to parse STT pricing extraction for ${pricingProvider}:`, text.slice(0, 500));
    return [];
  }
}
