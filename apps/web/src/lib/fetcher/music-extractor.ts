...
import { MUSIC_SYSTEM_PROMPT } from './music-system-prompt';

export interface ExtractedMusicModel {
  modelId: string;
  displayName: string;
...
export async function extractMusicPricing(
  pricingProvider: string,
  pageText: string
): Promise<MusicExtractionOutput> {
  ...
  const userPrompt = `Extract ${pricingProvider} music generation model pricing from this page:\n\n${truncated}`;

  const startTime = Date.now();
  let result: ExtractionResult;
  try {
    result = await extractionProvider.extract(apiKey, config.extractionModel, MUSIC_SYSTEM_PROMPT, userPrompt);
  } catch (err) {
    logUsage({
      ...
      metadata: { pricingProvider, operation: 'music_extraction' },
    });
    throw new Error(
      `Music extraction failed with ${config.extractionProvider}/${config.extractionModel}: ${err instanceof Error ? err.message : String(err)}`
    );
  }

  const models = parseMusicModels(result.content, pricingProvider);

  logUsage({
    provider: config.extractionProvider,
    model: config.extractionModel,
    operation: 'music_extraction',
...
function parseMusicModels(text: string, pricingProvider: string): ExtractedMusicModel[] {
  try {
    ...
    console.warn(`Failed to parse music pricing extraction for ${pricingProvider}:`, text.slice(0, 500));
    return [];
  }
}
