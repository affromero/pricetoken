...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = `pt:cache:avatar:compare:${modelIds.sort().join(',')}`;

    const cached = await getCached<AvatarModelPricing[]>(cacheKey);
    let data = cached ?? await compareAvatarModels(modelIds);
    ...
    console.error('GET /api/v1/avatar/compare error:', err);
