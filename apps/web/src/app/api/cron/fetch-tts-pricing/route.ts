...
export async function GET(request: Request) {
  ...
  try {
    ...
    const verificationCost = await getRecentTtsVerificationCost();
    ...
    await deleteByPattern('pt:cache:tts:*').catch(() => {});
    ...
    return apiError(`TTS fetch failed: ${message}`, 500);
  }
}

async function getRecentTtsVerificationCost(): Promise<number> {
