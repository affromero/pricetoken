...
import styles from './AvatarModelCompare.module.css';

interface AvatarModelCompareProps {
  pricing: AvatarModelPricing[];
}

const MAX_COMPARE = 5;

export function AvatarModelCompare({ pricing: initialPricing }: AvatarModelCompareProps) {
        ...
        <div className={styles.comparison}>
          ...
          <div className={styles.section}>
            ...
            <div className={styles.detailsGrid}>
              ...
              <div className={styles.detailHeader}>Type</div>
              ...
              {selectedModels.map((m) => (
                <div key={m.modelId} className={styles.detailRow}>
                  ...
                  <div className={styles.detailCell}>{m.avatarType ?? '—'}</div>
                  <div className={styles.detailCell}>{m.resolution ?? '—'}</div>
                  <div className={styles.detailCell}>
                    {m.maxDuration ? `${m.maxDuration}s` : '—'}
                  </div>
                  <div className={styles.detailCell}>{m.qualityMode ?? '—'}</div>
                </div>
              ))}
            </div>
          </div>
