...
import styles from './TtsModelCompare.module.css';

interface TtsModelCompareProps {
  pricing: TtsModelPricing[];
}

const MAX_COMPARE = 5;

export function TtsModelCompare({ pricing: initialPricing }: TtsModelCompareProps) {
  ...
  const maxCostPerMChars = Math.max(...selectedModels.map((m) => m.costPerMChars), 0.01);
      ...
      {selectedModels.length > 0 && (
        <div className={styles.comparison}>
          <div className={styles.section}>
            <h3 className={styles.sectionTitle}>Cost per 1M Characters ({currencySymbol}/1M chars)</h3>
              ...
              <div key={m.modelId} className={styles.barRow}>
                ...
                <div className={styles.barTrack}>
                  <div
                    ...
                    style={{ width: `${(m.costPerMChars / maxCostPerMChars) * 100}%` }}
                  />
                </div>
                <span className={styles.barValue}>{currencySymbol}{m.costPerMChars.toFixed(2)}</span>
              </div>
            ))}
          </div>
          ...
          <div className={styles.section}>
            ...
            <div className={styles.detailsGrid}>
              ...
              <div className={styles.detailHeader}>Voice Type</div>
              <div className={styles.detailHeader}>Max Chars</div>
              <div className={styles.detailHeader}>Languages</div>
              <div className={styles.detailHeader}>Cost/1M</div>
              {selectedModels.map((m) => (
                <div key={m.modelId} className={styles.detailRow}>
                  <div className={styles.detailCell}>{m.displayName}</div>
                  <div className={styles.detailCell}>{m.voiceType ?? '—'}</div>
                  <div className={styles.detailCell}>{formatMaxChars(m.maxCharacters)}</div>
                  <div className={styles.detailCell}>{m.supportedLanguages ?? '—'}</div>
                  <div className={styles.detailCell}>{currencySymbol}{m.costPerMChars.toFixed(2)}</div>
                </div>
              ))}
            </div>
          </div>
