...
import { calculateVideoCost } from 'pricetoken';
...
import styles from './VideoCostCalculator.module.css';

interface VideoCostCalculatorProps {
  pricing: VideoModelPricing[];
}

export function VideoCostCalculator({ pricing: initialPricing }: VideoCostCalculatorProps) {
  ...
  const handleCurrencyChange = useCallback(async (code: string) => {
    ...
    try {
      const res = await fetch(`/api/v1/video?currency=${code}`);
      if (res.ok) {
  ...
  const resolutions = [...new Set(pricing.map((m) => m.resolution).filter(Boolean))] as string[];

  const filteredPricing = useMemo(() => {
    let result = pricing;
    if (providerFilter) result = result.filter((m) => m.provider === providerFilter);
    if (resolutionFilter) result = result.filter((m) => m.resolution === resolutionFilter);
    return result;
  }, [pricing, providerFilter, resolutionFilter]);
  ...
  const rankedModels = useMemo(() => {
    return filteredPricing
      .map((m) => ({
        model: m,
        cost: calculateVideoCost(m.modelId, m.costPerMinute, durationSeconds),
      }))
    ...
    <div className={styles.root}>
      <div className={styles.hero}>
        <label className={styles.heroLabel} htmlFor="duration-slider">
          How much for a <span className={styles.heroAccent}>{durationMinutes.toFixed(1)}-minute</span> video?
        </label>
        <input
          id="duration-slider"
          type="range"
      ...
      <div className={styles.controls}>
        ...
        <div className={styles.controlsRow}>
          <div className={styles.resolutionFilter}>
            <button
              className={`${styles.toggle} ${!resolutionFilter ? styles.toggleActive : ''}`}
              onClick={() => setResolutionFilter('')}
            >
              All
            </button>
            {resolutions.map((r) => (
              <button
                key={r}
                className={`${styles.toggle} ${resolutionFilter === r ? styles.toggleActive : ''}`}
          ...
          return (
            <div key={model.modelId} className={styles.modelRow}>
              <div className={styles.modelInfo}>
                ...
                {model.resolution && (
                  <span className={styles.modelMeta}>{model.resolution}</span>
                )}
              </div>
