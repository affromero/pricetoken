...
import styles from './SttPricingTable.module.css';

interface SttPricingTableProps {
  pricing: SttModelPricing[];
}

type SortKey = 'provider' | 'displayName' | 'costPerMinute' | 'sttType' | 'maxDuration' | 'confidenceScore' | 'launchDate';

export function SttPricingTable({ pricing: initialPricing }: SttPricingTableProps) {
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    if (sortKey === 'sttType') {
      const aVal = a.sttType ?? '';
      const bVal = b.sttType ?? '';
      return sortAsc ? aVal.localeCompare(bVal) : bVal.localeCompare(aVal);
    }
    if (sortKey === 'maxDuration') {
      const aVal = a.maxDuration ?? 0;
      const bVal = b.maxDuration ?? 0;
  ...
  function formatSttType(type: string | null): string {
    if (!type) return '—';
        ...
        <table className={styles.table}>
          <thead>
            <tr>
              ...
              <th onClick={() => handleSort('sttType')} className={styles.sortable}>
                STT Type {sortKey === 'sttType' ? (sortAsc ? '↑' : '↓') : ''}
              </th>
              <th onClick={() => handleSort('maxDuration')} className={styles.sortable}>
                Max Duration {sortKey === 'maxDuration' ? (sortAsc ? '↑' : '↓') : ''}
              </th>
              <th>Languages</th>
