...
import styles from './TtsPriceHistoryChart.module.css';

interface TtsPriceHistoryChartProps {
...
export function TtsPriceHistoryChart({ history }: TtsPriceHistoryChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    return sortedDates.map((date) => {
      ...
      for (const model of filteredHistory) {
        ...
        if (match) {
          point[model.modelId] = match.costPerMChars * exchangeRate;
        }
      }
      return point;
    });
        ...
        {(width) => (
          <LineChart width={width} height={mobile ? 280 : 400} data={chartData} margin={mobile ? { top: 5, right: 10, bottom: 5, left: 0 } : { top: 5, right: 20, bottom: 5, left: 10 }}>
            ...
            <Tooltip
              ...
              formatter={(value: number, name: string) => [`${currencySymbol}${value.toFixed(2)}/1M chars`, name]}
            />
