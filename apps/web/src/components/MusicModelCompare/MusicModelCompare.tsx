...
import styles from './MusicModelCompare.module.css';

interface MusicModelCompareProps {
  pricing: MusicModelPricing[];
}

const MAX_COMPARE = 5;

export function MusicModelCompare({ pricing: initialPricing }: MusicModelCompareProps) {
  ...
  const handleCurrencyChange = useCallback(async (code: string) => {
    ...
    try {
      const res = await fetch(`/api/v1/music/compare?currency=${code}`);
      if (res.ok) {
        ...
        <div className={styles.comparison}>
          <div className={styles.section}>
            ...
            {selectedModels.map((m) => (
              <div key={m.modelId} className={styles.barRow}>
                ...
                <span className={styles.barValue}>{currencySymbol}{m.costPerMinute.toFixed(2)}</span>
              </div>
            ))}
          </div>
          ...
          <div className={styles.section}>
            ...
            <div className={styles.detailsGrid}>
              ...
              <div className={styles.detailHeader}>Format</div>
              <div className={styles.detailHeader}>Vocals</div>
              {selectedModels.map((m) => (
                <div key={m.modelId} className={styles.detailRow}>
                  <div className={styles.detailCell}>{m.displayName}</div>
                  <div className={styles.detailCell}>{currencySymbol}{m.costPerMinute.toFixed(3)}</div>
                  <div className={styles.detailCell}>
                    {m.maxDuration ? `${m.maxDuration}s` : '—'}
                  </div>
                  <div className={styles.detailCell}>{m.outputFormat ?? '—'}</div>
                  <div className={styles.detailCell}>
                    {m.vocals == null ? '—' : m.vocals ? 'Yes' : 'No'}
                  </div>
                </div>
              ))}
            </div>
