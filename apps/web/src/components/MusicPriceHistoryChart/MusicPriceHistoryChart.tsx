...
import styles from './MusicPriceHistoryChart.module.css';

interface MusicPriceHistoryChartProps {
...
export function MusicPriceHistoryChart({ history }: MusicPriceHistoryChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    return sortedDates.map((date) => {
      ...
      for (const model of filteredHistory) {
        ...
        if (match) {
          point[model.modelId] = match.costPerMinute * exchangeRate;
        }
      }
      return point;
    });
  }, [filteredHistory, exchangeRate]);
        ...
        {(width) => (
          <LineChart width={width} height={mobile ? 280 : 400} data={chartData} margin={mobile ? { top: 5, right: 10, bottom: 5, left: 0 } : { top: 5, right: 20, bottom: 5, left: 10 }}>
            ...
            <YAxis
              ...
              width={mobile ? 40 : 60}
              tickCount={5}
              label={mobile ? undefined : { value: '$/min', angle: -90, position: 'insideLeft', offset: -5, style: { fontSize: 11, fill: 'var(--pt-text-secondary)' } }}
            ...
            <Tooltip
              ...
              formatter={(value: number, name: string) => [`${currencySymbol}${value.toFixed(2)}/min`, name]}
            />
