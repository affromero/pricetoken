...
import styles from './ImagePriceHistoryChart.module.css';

interface ImagePriceHistoryChartProps {
...
export function ImagePriceHistoryChart({ history }: ImagePriceHistoryChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    return sortedDates.map((date) => {
      ...
      for (const model of filteredHistory) {
        ...
        if (match) {
          point[model.modelId] = match.pricePerImage * exchangeRate;
        }
      }
      return point;
    });
          ...
          <LineChart width={width} height={mobile ? 280 : 400} data={chartData} margin={mobile ? { top: 5, right: 10, bottom: 5, left: 0 } : { top: 5, right: 20, bottom: 5, left: 10 }}>
            ...
            <YAxis
              ...
              tickFormatter={(v: number) =>
                mobile
                  ? `${currencySymbol}${v < 0.01 ? v.toFixed(3) : v.toFixed(2)}`
            ...
            <Tooltip
              ...
              formatter={(value: number, name: string) => [`${currencySymbol}${value.toFixed(3)}/img`, name]}
            />
