...
import styles from './SttPriceHistoryChart.module.css';

interface SttPriceHistoryChartProps {
...
export function SttPriceHistoryChart({ history }: SttPriceHistoryChartProps) {
          ...
          <LineChart width={width} height={mobile ? 280 : 400} data={chartData} margin={mobile ? { top: 5, right: 10, bottom: 5, left: 0 } : { top: 5, right: 20, bottom: 5, left: 10 }}>
            ...
            <YAxis
              ...
              tickFormatter={(v: number) =>
                mobile
                  ? `${currencySymbol}${v < 1 ? v.toFixed(3) : v.toFixed(2)}`
            ...
            <Tooltip
              ...
              formatter={(value: number, name: string) => [`${currencySymbol}${value.toFixed(4)}/min`, name]}
            />
